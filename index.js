...
function wikipediaOpenSearch(lang, query) {
    ...
    return instance.get(
        `https://${lang}.wikipedia.org/w/api.php?action=opensearch&search=${query}&format=json`
    );
}
...
function wikipediaParse(lang, query) {
    query = encodeURIComponent(query);
    return instance.get(
        `https://${lang}.wikipedia.org/w/api.php?action=parse&page=${query}&format=json&prop=wikitext`
    );
}
...
async function wikipediaLangLink(lang, query, newLang) {
    query = encodeURIComponent(query);
    return instance.get(
        `https://${lang}.wikipedia.org/w/api.php?action=query&format=json&prop=langlinks&titles=${query}&lllang=${newLang}`
    );
}
...
function formatPopulationCount(txt) {
    ...
    if (txt.includes('tys')) {
        ...
        txt = txt.replaceAll(',', '.');
        txt = String(parseInt(txt * 1000, 10));
    }
    txt = txt.replaceAll('&', '');
...
function checkIfAllRes(odsData, responseCount, rows, sourceWeight, sourceLang) {
    if (
        typeof odsData[responseCount] === 'undefined' ||
        typeof odsData[responseCount][lpField] === 'undefined'
    ) {
        let c = 0;
        const flag1 = [];
		/*
    ...
    */
        rows.forEach((element, i) => {
            ...
            wikipediaParse(sourceLang[i], element).then(({ data }) => {
                ...
                const zrodlo = `https://${sourceLang[i]}.wikipedia.org/wiki/${element.replaceAll(
                    ' ',
                    '_'
                )}`;
                ...
                res[i] = {
                    p,
                    ll: ll === natxt ? ll : formatPopulationCount(ll),
                    lld: lld === natxt ? lld : formatPopulationYear(lld),
                    mnpm,
                    dz,
                    pm,
                    zrodlo,
                ...
                if (
                    ...
                    if (sourceLang[i] === 'pl') {
                        res[i].addLang === 'en';
                        wikipediaLangLink('pl', element, 'en').then(({ data }) => {
                            ...
                            if (typeof data.query.pages[pageId].langlinks !== 'undefined') {
                                ...
                                const newLangName = data.query.pages[pageId].langlinks[0]['*'];
                                wikipediaParse('en', newLangName).then(({ data }) => {
                                    data.parse.wikitext = data.parse.wikitext['*'];
                                    regExpLL = regEx.en.ll;
                                    regExpLLD = regEx.en.lld;
                                    regExpP = regEx.en.p;
                                    ...
                                    regExpDZ = regEx.en.dz;
                                    regExpMNPM = regEx.en.mnpm;
                                    const ll = regExpLL.exec(data.parse.wikitext);
                                    const lld = regExpLLD.exec(data.parse.wikitext);
                                    const p = regExpP.exec(data.parse.wikitext);
                                    const pm = regExpPM.exec(data.parse.wikitext);
                                    const dz = regExpDZ.exec(data.parse.wikitext);
                                    const mnpm = regExpMNPM.exec(data.parse.wikitext);
                                    ...
                                    if (llUsed || lldUsed || pUsed || pmUsed || dzUsed || dzUsed || mnpmUsed) {
                                        res[i].zrodlo += `, https://en.wikipedia.org/wiki/${newLangName.replaceAll(
                                            ' ',
                                            '_'
                                        )}`;
                                    }
                        ...
                        res[i].addLang === 'pl';
                        wikipediaLangLink('en', element, 'pl').then(({ data }) => {
                            const pageId = Object.keys(data.query.pages)[0];
                            if (typeof data.query.pages[pageId].langlinks !== 'undefined') {
                                // console.log(data.query.pages[pageId].langlinks[0]['*']);
                                // element = data.query.pages[pageId]
                                ...
                                wikipediaParse('en', newLangName).then(({ data }) => {
                                    ...
                                    if (llUsed || lldUsed || pUsed || pmUsed || dzUsed || dzUsed || mnpmUsed) {
                                        res[i].zrodlo += `, https://pl.wikipedia.org/wiki/${newLangName.replaceAll(
                                            ' ',
                                            '_'
                                        )}`;
                                    }
                    ...
                    .then(({ data }) => {
                        if (data[1].length > 0) {
                            ...
                            data[2].forEach((articleDesc, indexCurrent) => {
                                if (
                                    (articleDesc.includes('miasto') ||
                                        articleDesc.includes('miejscowość') ||
                                        articleDesc.includes('osiedle') ||
                                        articleDesc.includes('stolica') ||
                                        articleDesc.includes('dyskryt')) &&
                                    articleDesc.includes(odsRow.kraj) &&
                                    articleDesc.includes(odsRow.ja1) &&
                                    !articleDesc.includes('film') &&
                                    !articleDesc.includes('album') &&
                                    !articleDesc.includes('utwór')
                                ) {
                                    if (typeof sourceWeight[index] === 'undefined') {
                                        rows[index] = data[1][indexCurrent];
                                        sourceWeight[index] = 0;
                                    } else if (sourceWeight[index] > 0) {
                                        rows[index] = data[1][indexCurrent];
                                        sourceWeight[index] = 0;
                                    }
                                } else if (
                                    (articleDesc.includes('miasto') ||
                                        ...
                                        articleDesc.includes('stolica') ||
                                        articleDesc.includes('dyskryt')) &&
                                    articleDesc.includes(odsRow.kraj.substr(0, 4)) &&
                                    articleDesc.includes(odsRow.ja1) &&
                                    !articleDesc.includes('film') &&
                                    !articleDesc.includes('album') &&
                                    !articleDesc.includes('utwór')
                                    ...
                                    !articleDesc.includes('album') &&
                                    !articleDesc.includes('utwór') &&
                                    !articleDesc.includes('książ')
                                ) {
                                    if (typeof sourceWeight[index] === 'undefined') {
                                        sourceWeight[index] = 2;
                                        rows[index] = data[1][indexCurrent];
                                    } else if (sourceWeight[index] > 2) {
                                        rows[index] = data[1][indexCurrent];
                                        sourceWeight[index] = 2;
                                    }
                                } else if (
                            ...
                            responseCount--;
                            wikipediaOpenSearch('en', odsRow.miasto).then(({ data }) => {
                                if (data[1].length > 0) {
                                    sourceLang[index] = 'en';
                                    data[2].forEach((articleDesc, indexCurrent) => {
                                        if (
                                            (articleDesc.includes('town') ||
                                                articleDesc.includes('city') ||
                                                articleDesc.includes('district')) &&
                                            articleDesc.includes(odsRow.kraj) &&
                                            articleDesc.includes(odsRow.ja1) &&
                                            !articleDesc.includes('movie') &&
                                            !articleDesc.includes('album') &&
                                            !articleDesc.includes('song') &&
                                            !articleDesc.includes('book')
                                        ) {
                                            if (typeof sourceWeight[index] === 'undefined') {
                                                rows[index] = data[1][indexCurrent];
                                                sourceWeight[index] = 0;
...
app.get('/', (req, resp) => {
    ...
    for (let i = 0; i <= recordsCount - 1; i++) {
        if (typeof res[i] === 'undefined') {
            ...
            str += `${formatBasic(res[i].p)};${formatBasic(res[i].ll)};${formatBasic(
                res[i].lld
            )};${formatBasic(res[i].mnpm)};${formatBasic(res[i].dz)};${formatBasic(res[i].pm)};${
                res[i].zrodlo
                };;${res[i].f1}<br />`;
        }
    }
